...
from phase13_world_model_train import run_phase13_training, write_phase13_outputs
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run Phase 13 formal multi-scene AMF World Model training.")
    ...
    if unsupported:
        raise ValueError(f"Unsupported horizons for Phase 13: {unsupported}")
    ...
    if not scene_shards:
        raise FileNotFoundError("No cached PhysicalAI physics shards selected for Phase 13.")

    result = run_phase13_training(
        scene_shards,
    ...
    write_phase13_outputs(result, Path(args.out_dir))
    print(
        {
            "out_json": str(Path(args.out_dir) / "phase13_latest.json"),
            "scenes": result["scenes"],
            "passed": result["success_audit"]["phase13_passed"],
        }
    )

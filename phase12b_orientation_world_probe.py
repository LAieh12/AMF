...
def normalized_quat(track: PhysicsTrack, frame: int) -> np.ndarray:
    if track.rot is None:
        return np.asarray([1.0, 0.0, 0.0, 0.0], dtype=np.float32)
    quat = track.rot[frame].astype(np.float32)
    norm = float(np.linalg.norm(quat))
    return quat / max(norm, 1e-6)


def orientation_delta(track: PhysicsTrack, frame: int) -> np.ndarray:
    quat = normalized_quat(track, frame)
    prev = normalized_quat(track, max(0, frame - 1))
    delta = quat - prev
    angular_change = float(np.linalg.norm(delta))
    return np.concatenate([delta, np.asarray([angular_change], dtype=np.float32)]).astype(np.float32)


def orientation_feature(track: PhysicsTrack, frame: int, horizon: int) -> np.ndarray:
    dt = horizon / FPS
    com = track.com[frame]
    ...
    return np.concatenate(
        [
            ...
            normalized_quat(track, frame),
            orientation_delta(track, frame),
        ]
    ).astype(np.float32)
    ...
    for track in tracks:
        ...
        for horizon in horizons:
            for frame in range(0, frames - horizon, stride):
                ...
                by_h[horizon].append((orientation_feature(track, frame, horizon), target, com, cv))
...
def run_orientation_probe(
    ...
    fit_samples = make_orientation_samples(tracks, fit_sequences, HORIZONS, stride)
    val_samples = make_orientation_samples(tracks, val_sequences, HORIZONS, stride)
    train_samples = make_orientation_samples(tracks, train_sequences, HORIZONS, stride)
    test_samples = make_orientation_samples(tracks, test_sequences, HORIZONS, stride)
    ...
    return {
        "probe": "phase12b_orientation_world_probe",
...
def render_report(result: dict[str, Any]) -> str:
    ...
    return base.replace(
        ...
        "La arquitectura activa combina encoder fisico enriquecido, identidad de slot, rotacion/orientacion, Ridge global y memorias AMF locales normalizadas para corregir residuales.",
    )


def main() -> None:
    parser = argparse.ArgumentParser(description="Train/evaluate an orientation-aware AMF PhysicalAI world probe.")
    ...
    parser.add_argument("--out-json", default="results/phase12b_orientation_world_probe.json")
    parser.add_argument("--out-report", default="results/FASE12B_ORIENTATION_WORLD_PROBE.md")

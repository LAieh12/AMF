...
from phase12b_orientation_world_probe import normalized_quat, orientation_delta
...
def energy_feature(track: PhysicsTrack, frame: int, horizon: int) -> np.ndarray:
    dt = horizon / FPS
    ...
    tangential = velocity - radial_velocity * radial_dir
    speed = float(np.linalg.norm(velocity))
    tangential_speed = float(np.linalg.norm(tangential))
    height = float(com[1] - anchor[1])
    kinetic = speed * speed
    potential = 9.81 * height
    spin_energy = float(np.linalg.norm(spin / 1000.0) ** 2)
    ...
    for track in tracks:
        ...
        for horizon in horizons:
            for frame in range(0, frames - horizon, stride):
                ...
                by_h[horizon].append((energy_feature(track, frame, horizon), target, com, cv))

    out = {}
    for horizon, rows in by_h.items():
        if not rows:
            out[horizon] = (
                np.zeros((0, 53), dtype=np.float32),
...
def run_energy_probe(
    ...
    fit_samples = make_energy_samples(tracks, fit_sequences, HORIZONS, stride)
    val_samples = make_energy_samples(tracks, val_sequences, HORIZONS, stride)
    train_samples = make_energy_samples(tracks, train_sequences, HORIZONS, stride)
    test_samples = make_energy_samples(tracks, test_sequences, HORIZONS, stride)
    ...
    return {
        "probe": "phase12c_energy_world_probe",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train/evaluate an energy-aware AMF PhysicalAI world probe.")
    ...
    parser.add_argument("--out-json", default="results/phase12c_energy_world_probe.json")
    parser.add_argument("--out-report", default="results/FASE12C_ENERGY_WORLD_PROBE.md")
    args = parser.parse_args()

    result = run_energy_probe(
        Path(args.tar_path),
        args.train_fraction,
        args.stride,
        args.max_cells,
